...
do
    rm -rf TOTVAR_REWEIGHT_${pH} ffchi_${pH}_tot.dat ffeRMSD_${pH}_tot.dat
     cat > plumed_wham_${pH}.dat << EOF
...
# Activate MOLINFO functionalities
    ...
    DUMPGRID GRID=ffchi FILE=ffchi_${pH}_tot.dat 
    DUMPGRID GRID=ffpuck FILE=ffpuck_${pH}_tot.dat 

# Make sure you print out the 3 variables in the specified order at every step
PRINT ARG=chi,puck.Zx,metad.bias FILE=TOTVAR_REWEIGHT_${pH} STRIDE=1 # <- also change this one! 
EOF
	
	plumed driver --mf_xtc data/${sys}_tot.xtc --plumed plumed_wham_${pH}.dat --kt 2.5
done
